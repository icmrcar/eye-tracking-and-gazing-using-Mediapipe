...
LOGIC 1 -- STRICT / SAFE hold-to-activate motion state machine.
...
Rules:
    ...
    - While FORWARD: CENTER keeps driving forward (does not stop).
      LEFT/RIGHT glances are IGNORED -- turning is only reachable from a
      full stop in this strict variant.
    - While TURNING: the held direction must be maintained continuously
      (same flicker tolerance). Releasing to CENTER (past tolerance) stops
      the turn IMMEDIATELY -> STOPPED. Resuming forward afterward requires
      a fresh 3-second UP hold from STOPPED, exactly like any other
      activation -- no shortcuts.
    - A long blink or double blink always forces STOPPED immediately,
      from any state, and clears any activation hold in progress.
    - A beep plays the instant any movement activates.
    - Speed ramps up gradually after activation (speed_ramp.py), not an
      instant jump to full speed.
 
Run this to test Logic 1 on the real wheelchair. Run main_logic2.py
separately to test the alternative (smooth) design, then compare.
...
# ---------------------------------------------------------------------
# Beep cue (plays on the exact frame a movement activates)
# ---------------------------------------------------------------------
...
def _init_beep() -> None:
    ...
    if not _PYGAME_AVAILABLE:
        print("[main_logic1.py] NOTE: pygame not installed -- activation beep disabled.")
        return
    try:
        ...
        print(f"[main_logic1.py] NOTE: Beep unavailable ({e}).")
        _beep_sound = None
...
class GazeMotionFSM:
    """LOGIC 1 -- strict: every movement change passes through STOPPED."""
    ...
    def update(self, raw_direction, blink_stop_triggered):
        ...
        if self.state == MotionState.FORWARD:
            # Strict logic: LEFT/RIGHT glances are ignored while moving
            # forward -- turning is only reachable from a full stop.
            return self.state
        ...
        if self.state in (MotionState.TURN_LEFT, MotionState.TURN_RIGHT):
            ...
            if raw_direction == required:
                ...
                if self._turn_off_since is None:
                    ...
                    # Held direction released past tolerance -- turn stops
                    # immediately, and STRICTLY goes to STOPPED (not back
                    # to FORWARD). Resuming forward needs a fresh 3-second
                    # UP hold from STOPPED.
                    self._turn_off_since = None
                    self.state = MotionState.STOPPED
            return self.state
...
def log(message: str) -> None:
    if config.DEBUG:
        print(f"[main_logic1.py] {message}")
...
def main():
    ...
    log(f"Entering main control loop (LOGIC 1 -- strict). "
    ...
    while running:
        try:
            ...
            if hard_stop:
                ...
                # Manual joystick fully overrides the gaze state machine
                # and keeps it synced to STOPPED so gaze starts fresh once
                ...
                # Mid-blink, not yet stop-worthy -- don't feed a
                # potentially corrupted direction into the FSM this frame.
            ...
            if command != last_command:
                candidate, elapsed_ms, required_ms = motion_fsm.activation_progress(time.time())
